...
# Get the pane PID and check if uv run oobot is running
PANE_PID=$(tmux list-panes -t "$TARGET" -F '#{pane_pid}')

is_oobot_running() {
    pstree -a "$PANE_PID" 2>/dev/null | grep -q 'uv.*run oobot\|oobot.*\.venv/bin/oobot'
}

# Stop existing process if running
if is_oobot_running; then
    echo "Found running oobot process, sending Ctrl-C..."
    tmux send-keys -t "$TARGET" C-c

    # Wait for process to exit
    waited=0
    while is_oobot_running && [ "$waited" -lt "$MAX_WAIT" ]; do
        sleep 1
        waited=$((waited + 1))
    ...
    echo "No oobot process running in $TARGET"
fi
...
# Start oobot
echo "Starting oobot in $TARGET..."
tmux send-keys -t "$TARGET" "cd ${PROJECT_DIR} && uv run oobot" Enter

# Verify startup and show logs
sleep 3
if is_oobot_running; then
    echo "oobot restarted successfully. Recent logs:"
    echo "----------------------------------------"
    tmux capture-pane -t "$TARGET" -p | tail -20
    echo "----------------------------------------"
else
    echo "Warning: oobot may not have started. Pane output:"
    echo "----------------------------------------"
